...
const Home: NextPage = () => {
  ...
  const [programName] = useState<string>("jimmy_multiplication");
  ...
  const [parties] = useState<string[]>(["Party1", "Party2"]);
  const [outputs] = useState<string[]>(["multiplication_result"]);
  ...
  async function handleRetrieveInt(secret_name: string, store_id: string | null) {
    if (store_id) {
      ...
      console.error("store_id is null");
    }
  }
  ...
  async function handleSecretFormSubmit(
    ...
    if (programId) {
      const partyName = secretName === 'secret_int1' ? parties[0] : parties[1];
      await storeSecretsInteger(
        ...
        console.error("Failed to store secret integer:", error);
      });
    }
  }
  ...
  async function handleCompute() {
    if (programId) {
      await compute(nillion, nillionClient, Object.values(storedSecretsNameToStoreId), programId, outputs[0]).then(
        ...
        console.error("Failed to compute on secrets:", error);
      });
    }
  }
      ...
      <div className="flex items-center flex-col pt-10">
        <div className="px-5 flex flex-col">
          <h1 className="text-xl">
            <span className="block text-4xl font-bold">Demo: Secret Multiplication on Nillion</span>
